  ...
  const data = [
    {
      id: 1,
      label:
        'Anytime content is captured, created, or edited, Content Credentials can be attached.',
      labelNumber: 1,
      labelBg: '#EF9349',
      Image: Hiw1,
      description:
    ...
    {
      id: 2,
      label: 'Content Credentials are viewable across the internet',
      labelNumber: 2,
      labelBg: '#FCE458',
      Image: Hiw2,
      description:
...
<div
  class="flex h-full w-screen flex-col items-center bg-brand-gray p-[1.5rem] pb-40">
  <h2
    class="2xl:text-hiw-title-xxl pt-[4rem] text-hiw-title md:text-hiw-title-md xl:text-hiw-title-xl">
    How it Works
  </h2>
      ...
      {#each data as item}
        <div
          ...
          <div
            ...
            <p
              class="pt-6 text-hiw-desc text-gray-900/60 lg:w-1/2 lg:pt-0 xl:text-hiw-desc-xl 2xl:text-hiw-desc-xxl">
              {item.description}
            </p>
          </div>
          <img
            src={item.Image}
            alt="how it works"
            class="max-w[71.375rem] rounded-xxl 2xl:w-full" />
        </div>
      {/each}
      <!-- Video Component -->
      <div
